...
export default {
  ...
  methods: {
    ...
    deleteItem(item) {
      this.$http
        .delete(`${process.env.VUE_APP_API_VIOLATION}/${item}`)
        .then((res) => {
    ...
    editItem(item) {
      this.$router.push(`/admin/violation/${item._id}/edit`);
    },
    viewItem(item) {
      this.$router.push(`/admin/violation/${item._id}/view`);
    },
    getTableData() {
      return new Promise((resolve, reject) => {
        this.$http
          .get(
            `${process.env.VUE_APP_API_VIOLATION}/accepter/${this.$store.state.acceptor._id}`
          )
          .then((res) => {
