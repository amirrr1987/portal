...
export default {
  ...
  methods: {
    ...
    categoryList() {
      this.$http.get(process.env.VUE_APP_API_CATEGORY).then((res) => {
        this.parents = res.data.data.filter(
          (singleItem) =>
            singleItem._id != this.$route.params.id && singleItem.parent == null
        );
      });
    },
    //  End  category list
    ...
    getData() {
      this.$http
        .get(`${process.env.VUE_APP_API_CATEGORY}/${this.$route.params.id}`)
        .then((res) => {
          this.category = res.data.data;
          if (res.data.data.parent != null) this.categoryType = 2;
        })
    ...
    updateForm() {
      this.$http
        .put(
          `${process.env.VUE_APP_API_CATEGORY}/${this.$route.params.id}`,
          this.category
        )
        .then(() => {
          this.snackbar = true;
